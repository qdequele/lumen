...
- [x] Harnais de comparaison reproductible vs LiteLLM (docker-compose : mock amont + ferrogate + litellm + k6/oha) — mesurer latence ajoutée p50/p99, RAM, req/s
...
- [x] `docker run -v ./config.toml:/config.toml -e OPENAI_API_KEY ghcr.io/.../ferrogate` fonctionne tel quel
...
Note : métrique nommée `ferrogate_config_reload_failures_total` (+
`ferrogate_config_reloads_total`). Le reload préserve les clés provider
...
3. [x] Reload avec config cassée → service intact, `ferrogate_config_reload_failures_total` incrémentée (testé).
