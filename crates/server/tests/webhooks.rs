...
use lumen_server::config::{Config, WebhooksConfig};
use lumen_server::pricing::CostTable;
use lumen_server::webhooks::{spawn_webhook_sender, SigningKey, WebhookRuntime};
use lumen_server::AppState;
use lumen_telemetry::{LatencyMetrics, Metrics, TokenMetrics, WebhookMetrics};
...
/// stay quick.
fn webhook_config(receiver: &str, events: Vec<EventKind>, thresholds: Vec<u8>) -> WebhooksConfig {
    WebhooksConfig {
...
/// Spawn an auth-enabled gateway with the webhook sender wired up exactly as
...
async fn spawn_gateway(
    ...
    let webhook_metrics = WebhookMetrics::register(&metrics).expect("register webhook metrics");

    let (webhook_runtime, receiver, signals) =
        WebhookRuntime::build(webhooks, webhook_metrics.clone());
    runtime.keys.set_signals(Some(signals));
    let cancel = CancellationToken::new();
    spawn_webhook_sender(
        receiver,
        http::build_client(),
        webhook_runtime.policy(),
        signing_key,
        webhook_metrics,
        cancel.clone(),
    );
