...
Ferrogate is pre-1.0. Security fixes land on `main` and in the latest tagged
release. There is no long-term-support branch yet.

## Security model — what Ferrogate guarantees

Ferrogate is a self-hosted gateway; you run it inside your own trust boundary.
...
- **Secrets never leak.** Provider API keys are referenced by environment-variable
  *name* in config, never stored as values; when stored in the database (admin
  API) they are encrypted at rest with AES-256-GCM under `FERROGATE_MASTER_KEY`.
...
- **Virtual keys** are stored only as BLAKE3 hashes; the plaintext is shown once
  ...
  indistinguishable to the caller (`FG-4004`) so key state cannot be probed.
...
- **Terminate TLS** in front of Ferrogate (a reverse proxy / load balancer). The
  gateway speaks plain HTTP; do not expose it directly to the internet without
  TLS.
- **Protect `FERROGATE_MASTER_KEY`** and the SQLite database file — together they
