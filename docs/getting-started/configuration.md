...
A `SIGHUP`, a file watch (file mode), an admin config write, or an admin
provider-key rotation triggers a reload: the new config is validated, then
the provider registry, price table, resilience policy, the runtime-safe
`[auth]` knobs (`flush_interval_ms`, `retention_days`), the webhook policy,
the image-fetch policy and the tokenizer are atomically swapped.

The **boot layer** is read once at startup and needs a restart:
...
By default the whole config is the one TOML file the gateway was started
with (`config_source = "file"`). Setting `config_source = "db"` moves
everything outside the boot layer into the auth database instead, so the
gateway can be configured entirely through the admin API with no config
file to manage beyond a few boot keys. See
[Config source modes](../operations/config-modes.md).

## Viewing the live config over the admin API

`GET /admin/config` returns the current config document, verbatim: in file
mode, the config file the gateway booted from; in db mode, the dynamic
document stored in the database (an empty string before the first write).
It requires the master key AND `auth.enabled = true`: the whole `/admin/*`
router is only mounted when auth is on, so on a default deployment (where
`[auth]` is disabled) this route does not exist at all and answers 404
rather than 401.
...
merged in-memory config: `Config::load` overlays `LUMEN_*` environment
variables on top of it, and showing that merged view would make environment
overrides look like document content, and a later write would bake them in
permanently. `hash` is a BLAKE3 content hash of those same bytes, meant to
be echoed as `If-Match` on the `PUT /admin/config` that applies a new one
(ADR 010, ADR 012), so two operators editing at once cannot silently
clobber each other.

To read or change one provider or one section without handling the whole
...
thereby redirect customer traffic to a different upstream.** The granular
provider endpoints can do the same. Master key required, same as every
other `/admin/*` route, and likewise only mounted when `auth.enabled =
true`: with auth off the route is absent and returns 404, not 401.

The `If-Match` contract:

- Send the submitted document as the raw request body (`Content-Type` is
  irrelevant; the body is treated as the document's new contents).
...
- If `If-Match` does not equal the *current* document's hash (someone else
  applied a change since you last read it, or in file mode someone edited
  the file directly), the request is rejected with `412` (`LM-1004`) and
  nothing is written. `GET /admin/config` again to see what changed, then
  re-apply against the fresh hash.

What happens on a successful apply:

1. The candidate is validated exactly like a hot reload would: parsed,
   ...
   missing a required `base_url`) is rejected with `400` (`LM-1001`) and
   nothing is written. The error message names the offending field or
   setting (e.g. `"server.first_token_timeout_ms must not be 0"`) but never
   a filesystem path, which is logged server-side instead.
2. A candidate that changes a restart-only (boot-layer) setting is rejected
   with `400` (`LM-1001`), naming it: the route only accepts what it can
   apply immediately. In db mode, a candidate that carries any boot-layer
   key at all is rejected the same way.
3. The document is persisted atomically. File mode: the *current* file is
   copied to a `.bak` sibling (e.g. `lumen.toml.bak`) - one generation of
   history, enough to revert a bad apply by hand - and the new document is
   renamed into place. DB mode: a new row is added to `config_versions`
   (the newest 50 are kept).
4. The hot-reload trigger fires, so the new config takes effect without a
   restart (see [Hot reload](#hot-reload)). Every setting this route
   accepts is live once it returns `204`.

A rejected apply (`400` or `412`) is guaranteed to leave the stored
document byte-for-byte unchanged and to leave no temporary file behind.
Concurrent `PUT`s are serialised gateway-side, so two operators racing the
same pre-apply hash can never both land: exactly one wins, and the other
sees a `412` for a hash that moved out from under it, never a silently
corrupted mix of the two documents.
...
[Deployment](../operations/deployment.md#hot-reload)) on any file-mode
gateway that does not need remote apply. In db mode the admin API is the
only way to change the config, so there is no equivalent opt-out.
