...
| Code      | HTTP | Meaning                                                        |
|-----------|------|----------------------------------------------------------------|
| `LM-1001` | 400  | Malformed or invalid request body / parameters. Also returned when a provider in `strict` mode is sent an unsupported-but-meaningful field it cannot honor (e.g. `dimensions` to Ollama, or `response_format`/`seed`/`logprobs`/`parallel_tool_calls` to a translated chat provider that has no native equivalent - see the [chat-extras matrix](providers.md#openai-chat-extras-on-translated-providers)), and when an input *shape* a provider cannot consume at all is sent (pre-tokenized token-id arrays to a text-only embed API: Cohere, TEI, Ollama, Jina, Voyage, Mistral). Both are rejected before any upstream call; the message names the field/shape and provider. Also covers every rejection of a config write under `/admin/config*` (ADR 012, both `config_source` modes) that is not specifically `LM-1003` or `LM-1004`: a missing `If-Match` header on any mutating call, granular included; a candidate document that changes a restart-only boot-layer key (`server.*`, `log_format`, `telemetry.*`, `auth.enabled`, `auth.db_path`, the usage-log channel knobs, `config_source` itself) - the message names the changed key(s) and says "edit the boot config file and restart"; in `config_source = "db"` mode, a candidate that carries any boot-layer key at all, even at its default value (the message names the key); a candidate that fails validation (bad TOML, an unknown field, a dangling `fallbacks` reference); `PUT /admin/config/providers/{name}` where the path name and the body's own `name` disagree; `DELETE /admin/config/providers/{name}` while another model's `fallbacks` still references it, naming the dependent model; and a `PUT /admin/config/auth` body naming a boot-layer `[auth]` field (`enabled`, `db_path`, or a usage-log channel knob), since that route accepts only the 2 dynamic knobs (`flush_interval_ms`, `retention_days`). Also covers every `POST /v1/systemone` request-contract violation other than an empty `questions` map (that one is `LM-2011`): a `null` `state`, or a question with a missing or unknown `type`, missing `instructions`, or malformed `criteria` (see [SystemOne](systemone/systemone.md#validation-and-errors)); the message names the offending question. |
| `LM-1002` | 413  | Request body exceeded the configured size limit.               |
| `LM-1003` | 404  | No route matches the request method and path. Returned by the router fallback for trailing-slash, extra-segment and other near-miss paths, so an unmatched request carries the same envelope as every other rejection instead of a bare, empty-body 404. The message names no path and discloses no route. The fallback sits *outside* the virtual-key auth layer, so an unmatched path answers 404 even when auth is enabled (it leaks no more than the bare 404 did); a *matched* `/v1` route without a key is still `LM-4004`. Distinct from `LM-2001`, which means the HTTP route matched but the requested model id does not exist. Also returned by the granular config surface (ADR 012) for a provider name `GET`/`PUT`/`DELETE /admin/config/providers/{name}` does not recognize in the current document, and for a section name `GET`/`PUT /admin/config/{section}` outside the fixed set (`resilience`, `tokenizer`, `image_fetch`, `webhooks`, `auth`; `telemetry` is boot-layer and has no granular route) - the same per-entity-404 style every other admin lookup in LUMEN uses. |
| `LM-1004` | 412  | The submitted `If-Match` no longer matches the current config document's hash - the document changed since it was read, most often another operator's apply landing first (or, in `config_source = "file"` mode, an external edit: a human or a GitOps sync). Applies to every mutating call under `/admin/config*` (ADR 012): the whole-document `PUT /admin/config` and every granular write (`PUT`/`DELETE /admin/config/providers/{name}`, `PUT /admin/config/{section}`) alike, in both `config_source` modes. The hash is source-verbatim in both modes - file mode: the whole config file's bytes; DB mode: the stored `config_versions` document, or the empty document's hash before the first `PUT` - so `If-Match` semantics are identical either way and a client cannot tell which mode it is talking to. The request is refused before anything is staged, patched or written; re-`GET` the resource for the current hash and content, then re-apply. Kept distinct from a missing/malformed `If-Match` header, which is a plain `LM-1001` `400`: the console needs to tell "you sent no token" from "your token is stale" apart, since only the second case calls for a re-read-and-retry rather than a client bug fix. |
