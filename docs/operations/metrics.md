...
| `lumen_webhook_dead_total` | none | Budget events abandoned after exhausting their retry budget, or permanently rejected by the receiver. |
...
| `increase(lumen_webhook_dropped_total[5m]) > 0` | The webhook queue is full and budget signals are being shed: a billing backend relying on `budget.threshold` for auto-recharge is running blind. Raise `channel_capacity` or fix a slow receiver. |
